...
describe("hook ↔ prefetcher query-key parity", () => {
  ...
  it("detail — tenant-scoped (the old prefetchDetail bug: unscoped seed never hydrated)", () => {
    expect(hashKey(hookKeys.detail("p1", "org_1"))).toBe(
      hashKey(prefetchKeys.detail("p1", "org_1")),
    );
  });
...
describe("queryOptions factory ↔ hook key parity", () => {
  ...
  it("params-embedded organizationId wins over ctx (matches prefetcher precedence)", () => {
    expect(
      hashKey(Q.list({ organizationId: "org_A", limit: 5 }, { organizationId: "org_B" }).queryKey),
    ).toBe(hashKey(hookKeys.list("org_A", { limit: 5 })));
  });
  ...
  it("infiniteList carries real page semantics (offset hasNext -> page+1, keyset -> cursor)", () => {
    ...
    expect(
      opts.getNextPageParam(
        { method: "offset", hasNext: true, page: 2 } as never,
        [] as never,
        1 as never,
        [] as never,
      ),
    ...
    expect(
      opts.getNextPageParam(
        { method: "offset", hasNext: false, page: 9 } as never,
        [] as never,
        1 as never,
        [] as never,
      ),
    ...
    expect(
      opts.getNextPageParam(
        { method: "keyset", hasMore: true, next: "cur_9" } as never,
        [] as never,
        1 as never,
        [] as never,
      ),
