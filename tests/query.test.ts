...
describe("createQueryKeys", () => {
  ...
  it("creates scoped list key", () => {
    expect(KEYS.scopedList("tenant", { organizationId: "org-1" })).toEqual([
      "posts",
      "list",
      { _scope: "tenant", organizationId: "org-1" },
    ]);
  });
});
...
describe("createQueryKeys edge cases", () => {
  ...
  it("custom with multiple args", () => {
    expect(KEYS.custom("analytics", "monthly", "2024")).toEqual([
      "posts",
      "analytics",
      "monthly",
      "2024",
  ...
  it("scopedList with empty params", () => {
    expect(KEYS.scopedList("super-admin")).toEqual(["posts", "list", { _scope: "super-admin" }]);
  });

  it("scopedList with multiple params", () => {
    expect(
      KEYS.scopedList("tenant", { organizationId: "org-1", status: "active", page: 1 }),
    ).toEqual([
      "posts",
      "list",
      { _scope: "tenant", organizationId: "org-1", status: "active", page: 1 },
    ]);
  });
});
...
describe("Arc backend response shapes", () => {
  ...
  describe("BaseController.delete — delete response", () => {
    it("handles { success, data: { message, id, soft } }", () => {
      const response = {
        success: true,
        data: { message: "Deleted successfully", id: "1", soft: true },
      };
