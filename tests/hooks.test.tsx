...
import { configureAuth, configureClient, createClient } from "../src/client.js";
import type { CrudApi } from "../src/hooks.js";
import { configureNavigation, createCrudHooks } from "../src/hooks.js";
import { configureToast } from "../src/mutation.js";
import { createCacheUtils, createQueryKeys } from "../src/query.js";
...
describe("createCrudHooks", () => {
  ...
  let hooks: ReturnType<
    typeof createCrudHooks<{ _id: string; name: string }, { name: string }, { name: string }>
  >;
  ...
  describe("useList", () => {
    it("fetches list data", async () => {
      ...
      const { result } = renderHook(() => hooks.useList(null, {}, { public: true }), { wrapper });
    ...
    it("respects enabled option", async () => {
      ...
      renderHook(() => hooks.useList(null, {}, { public: true, enabled: false }), { wrapper });
    ...
    it("disables query without token for protected endpoints", async () => {
      ...
      renderHook(() => hooks.useList(null, {}, { public: false }), { wrapper });
    ...
    it("enables query with token for protected endpoints", async () => {
      ...
      renderHook(() => hooks.useList("my-token", {}), { wrapper });
  ...
  describe("useList with custom response keys", () => {
    it("extracts items from custom key like { products: [...] }", async () => {
      ...
      const wrapper = createWrapper(queryClient);
      const { result } = renderHook(() => customHooks.useList(null, {}, { public: true }), {
        wrapper,
      });
  ...
  describe("useDetail with custom response keys", () => {
    it("extracts item from { product: {...} }", async () => {
      ...
      const wrapper = createWrapper(queryClient);
      const { result } = renderHook(() => customHooks.useDetail("1", null, { public: true }), {
        wrapper,
      });
    ...
    it("extracts item from standard { data: {...} }", async () => {
      const wrapper = createWrapper(queryClient);
      const { result } = renderHook(() => hooks.useDetail("1", null, { public: true }), {
        wrapper,
      });
  ...
  describe("useDetail", () => {
    ...
    it("does not fetch without id", () => {
      ...
      renderHook(() => hooks.useDetail(null, null, { public: true }), { wrapper });
  ...
  describe("useActions", () => {
    ...
    it("remove calls api.delete", async () => {
      ...
      expect(mockApi.delete).toHaveBeenCalledWith(expect.objectContaining({ id: "1" }));
  ...
  describe("useNavigation", () => {
    ...
    it("calls router.push when configured", async () => {
      ...
      const { result } = renderHook(() => navHooks.useNavigation(), { wrapper });
  ...
  describe("custom messages", () => {
    it("uses custom success messages", async () => {
      ...
      const { result } = renderHook(() => customHooks.useActions(), { wrapper });
  ...
  describe("configureAuth / auto-injection", () => {
    ...
    it("useList auto-injects token/orgId from auth context (new signature)", async () => {
      ...
      renderHook(() => hooks.useList({ status: "active" }), { wrapper });
    ...
    it("useDetail auto-injects token from auth context (new signature)", async () => {
      ...
      renderHook(() => hooks.useDetail("1"), { wrapper });
    ...
    it("explicit token overrides auth context (legacy signature)", async () => {
      ...
      renderHook(() => hooks.useList("explicit-token", { organizationId: "explicit-org" }), {
        wrapper,
      });
    ...
    it("legacy useList(null, params, options) still works after configureAuth", async () => {
      ...
      renderHook(() => hooks.useList(null, { organizationId: "legacy-org" }, { public: true }), {
        wrapper,
      });
  ...
  describe("signal passthrough", () => {
    it("useList forwards AbortSignal to api.getAll for request cancellation", async () => {
      ...
      renderHook(() => hooks.useList(null, {}, { public: true }), { wrapper });
    ...
    it("useDetail forwards AbortSignal to api.getById for request cancellation", async () => {
      ...
      renderHook(() => hooks.useDetail("1", null, { public: true }), { wrapper });
  ...
  describe("useInfiniteList", () => {
    ...
    it("useInfiniteList auto-injects token from auth context (new signature)", async () => {
      ...
      renderHook(() => infiniteHooks.useInfiniteList({ status: "active" }), { wrapper });
  ...
  describe("cookie auth mode", () => {
    ...
    it("cookie mode + enabled: false still disables query", async () => {
      ...
      renderHook(() => hooks.useList({}, { enabled: false }), { wrapper });
  ...
  describe("polling and refetch configuration", () => {
    ...
    it("useList respects custom staleTime", async () => {
      const wrapper = createWrapper(queryClient);
      renderHook(() => hooks.useList(null, {}, { public: true, staleTime: 1000 }), { wrapper });
      await waitFor(() => {
        expect(mockApi.getAll).toHaveBeenCalled();
      });
    });
    ...
    it("useList respects refetchInterval for polling", async () => {
      const wrapper = createWrapper(queryClient);
      renderHook(() => hooks.useList(null, {}, { public: true, refetchInterval: 5000 }), {
        wrapper,
      });
      await waitFor(() => {
        expect(mockApi.getAll).toHaveBeenCalled();
      ...
      renderHook(
        () =>
          hooks.useList(
            null,
            {},
            { public: true, refetchInterval: 5000, refetchIntervalInBackground: true },
          ),
        { wrapper },
      );
      await waitFor(() => {
    ...
    it("useList respects refetchOnWindowFocus override", async () => {
      const wrapper = createWrapper(queryClient);
      renderHook(() => hooks.useList(null, {}, { public: true, refetchOnWindowFocus: true }), {
        wrapper,
      });
      await waitFor(() => {
        expect(mockApi.getAll).toHaveBeenCalled();
    ...
    it("useList disables polling with refetchInterval: false", async () => {
      const wrapper = createWrapper(queryClient);
      renderHook(() => hooks.useList(null, {}, { public: true, refetchInterval: false }), {
        wrapper,
      });
      await waitFor(() => {
    ...
    it("useList respects custom gcTime", async () => {
      const wrapper = createWrapper(queryClient);
      renderHook(() => hooks.useList(null, {}, { public: true, gcTime: 60_000 }), { wrapper });
      await waitFor(() => {
        expect(mockApi.getAll).toHaveBeenCalled();
      });
    });
    ...
    it("useDetail respects refetchInterval for polling", async () => {
      const wrapper = createWrapper(queryClient);
      renderHook(() => hooks.useDetail("1", null, { public: true, refetchInterval: 3000 }), {
        wrapper,
      });
      await waitFor(() => {
        expect(mockApi.getById).toHaveBeenCalled();
    ...
    it("useDetail respects refetchOnWindowFocus", async () => {
      const wrapper = createWrapper(queryClient);
      renderHook(() => hooks.useDetail("1", null, { public: true, refetchOnWindowFocus: true }), {
        wrapper,
      });
      await waitFor(() => {
        expect(mockApi.getById).toHaveBeenCalled();
    ...
    it("useInfiniteList respects refetchInterval", async () => {
      ...
      const wrapper = createWrapper(queryClient);
      renderHook(() => hooks.useInfiniteList(null, {}, { public: true, refetchInterval: 10_000 }), {
        wrapper,
      });
      await waitFor(() => {
        expect(mockApi.getAll).toHaveBeenCalled();
    ...
    it("createCrudHooks defaults.refetchOnWindowFocus is passed to useList", async () => {
      ...
      const wrapper = createWrapper(queryClient);
      renderHook(() => customHooks.useList(null, {}, { public: true }), { wrapper });
      await waitFor(() => {
        expect(mockApi.getAll).toHaveBeenCalled();
      });
    });
      ...
      renderHook(
        () =>
          customHooks.useList(
            null,
            {},
            { public: true, refetchOnWindowFocus: false, staleTime: 1000 },
          ),
        { wrapper },
      );
      await waitFor(() => {
    ...
    it("QUERY_CONFIGS.stable preset works with useDetail", async () => {
      ...
      const wrapper = createWrapper(queryClient);
      renderHook(() => stableHooks.useDetail("1", null, { public: true }), { wrapper });
      await waitFor(() => {
        expect(mockApi.getById).toHaveBeenCalled();
      });
    });
  ...
  describe("useActions silent option", () => {
    ...
    it("suppresses error toast when silent: true on failure", async () => {
      ...
      const { result } = renderHook(() => silentHooks.useActions(), { wrapper });
  ...
  describe("useActions update invalidates detail cache", () => {
    it("invalidates the details prefix at settle after successful update", async () => {
      const wrapper = createWrapper(queryClient);

      // Pre-populate detail cache (raw doc — arc 2.13+ has no envelope)
      queryClient.setQueryData(hooks.KEYS.detail("1"), { _id: "1", name: "Old" });
      ...
      // Settled last-standing invalidation covers every detail variant via
      // the details() prefix (bare + scoped + parameterized).
      expect(spy).toHaveBeenCalledWith({ queryKey: hooks.KEYS.details() });
    });
  });
  ...
  describe("onSettled callbacks", () => {
    it("calls onSettled after successful create (with extracted entity, not raw ApiResponse)", async () => {
      ...
      const { result } = renderHook(() => hooksWithCallbacks.useActions(), { wrapper });
  ...
  describe("optimistic update rollback", () => {
    it("rolls back list cache on create failure", async () => {
      ...
      const { result } = renderHook(() => rollbackHooks.useActions(), { wrapper });
  ...
  describe("infinite list signal passthrough", () => {
    it("useInfiniteList forwards AbortSignal to api.getAll", async () => {
      ...
      renderHook(() => infiniteHooks.useInfiniteList(null, {}, { public: true }), { wrapper });

      await waitFor(() => {
        expect(infiniteApi.getAll).toHaveBeenCalled();
        const callOptions = (infiniteApi.getAll as ReturnType<typeof vi.fn>).mock.calls[0]?.[0]
          .options;
        expect(callOptions.signal).toBeInstanceOf(AbortSignal);
      });
    });
  ...
  describe("query key structure", () => {
    it("scopedList includes _scope in key", () => {
      ...
      expect(key).toEqual([
        "items",
        "list",
        { _scope: "tenant", organizationId: "org-1", status: "active" },
      ]);
    });
  ...
  describe("multi-client support", () => {
    it("uses client toast handler instead of global", async () => {
      ...
      const { result } = renderHook(() => clientHooks.useActions(), { wrapper });
    ...
    it("uses client navigation hook instead of global", async () => {
      ...
      const { result } = renderHook(() => clientHooks.useNavigation(), { wrapper });
  ...
  describe("CallOptions.onSettled", () => {
    ...
    it("calls per-call onSettled after failed create", async () => {
      ...
      const { result } = renderHook(() => failHooks.useActions(), { wrapper });
  ...
  describe("useUpload", () => {
    it("rejects when api has no upload method", async () => {
      ...
      await act(async () => {
        await expect(result.current.mutateAsync({ data: new FormData() })).rejects.toThrow(
          '[arc-next] "items" api does not define an upload method',
        );
      });
    });
    ...
    it("calls upload api and shows toast", async () => {
      ...
      expect(uploadApi.upload).toHaveBeenCalledWith(expect.objectContaining({ data: formData }));
  ...
  describe("useCustomMutation", () => {
    ...
    it("invalidates default list keys", async () => {
      ...
      const { result } = renderHook(() => hooks.useCustomMutation({ mutationFn: customFn }), {
        wrapper,
      });
  ...
  describe("useDeleted", () => {
    it("fetches soft-deleted items", async () => {
      ...
      const wrapper = createWrapper(queryClient);
      const { result } = renderHook(() => deletedHooks.useDeleted({}, { public: true }), {
        wrapper,
      });
  ...
  describe("useDetailBySlug", () => {
    ...
    it("is disabled when slug is null", async () => {
      ...
      const wrapper = createWrapper(queryClient);
      renderHook(() => slugHooks.useDetailBySlug(null, { public: true }), { wrapper });
    ...
    it("disables query when api lacks getBySlug", () => {
      const wrapper = createWrapper(queryClient);
      const { result } = renderHook(() => hooks.useDetailBySlug("test", { public: true }), {
        wrapper,
      });
  ...
  describe("useTree", () => {
    it("fetches tree data", async () => {
      ...
      const wrapper = createWrapper(queryClient);
      const { result } = renderHook(() => treeHooks.useTree({}, { public: true }), { wrapper });
  ...
  describe("useChildren", () => {
    it("fetches children of a parent", async () => {
      const treeApi = {
        ...
        getChildren: vi.fn().mockResolvedValue({
          success: true,
          data: [
            { _id: "child-1", name: "Child 1" },
            { _id: "child-2", name: "Child 2" },
          ],
          total: 2,
          page: 1,
          limit: 10,
      ...
      const wrapper = createWrapper(queryClient);
      const { result } = renderHook(() => treeHooks.useChildren("parent-1", {}, { public: true }), {
        wrapper,
      });
    ...
    it("is disabled when parentId is null", async () => {
      ...
      const wrapper = createWrapper(queryClient);
      renderHook(() => treeHooks.useChildren(null, {}, { public: true }), { wrapper });
  ...
  describe("useActions — restore", () => {
    it("calls api.restore and shows success toast", async () => {
      ...
      expect(restoreApi.restore).toHaveBeenCalledWith(expect.objectContaining({ id: "del-1" }));
  ...
  describe("custom idField", () => {
    it("resolveItemId uses idField for optimistic create", async () => {
      ...
      skuApi.create = vi
        .fn()
        .mockResolvedValue({ _id: "mongo-1", sku: "SKU-001", name: "Product" });
    ...
    it("useDetail placeholderData finds items by idField (not _id) — list payload appears instantly under SKU key", async () => {
      ...
      const detail = renderHook(() => skuHooks.useDetail("SKU-001", { public: true }), {
        wrapper: localWrapper,
      });
    ...
    it("optimistic delete uses idField to match items", async () => {
      ...
      expect(skuApi.delete).toHaveBeenCalledWith(expect.objectContaining({ id: "DEL-001" }));
    });
  });
